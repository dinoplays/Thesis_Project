module epi_compiler #(
	...
	parameter int unsigned RAM_ADDR_W        = (2*$clog2(IMAGE_DIM)) + 2,
	parameter logic [RAM_ADDR_W-1:0] RAM_COLOUR_OFFSET = '0
)(
	input  wire                                   clk,
	...
	output logic [RAM_ADDR_W-1:0]                 storage_wr_addr [0:11],
	output logic [RAM_ADDR_W-1:0]                 storage_wr_addr_8v,
	output logic [14:0]                           storage_wr_data,
	output logic [RAM_ADDR_W-1:0]                 storage_rd_addr,
	...
	output logic [$clog2(IMAGE_DIM)-1:0]               epi_column_idx_out,
	output logic [$clog2(IMAGE_DIM)-1:0]               epi_idx_out,
	output logic                                  orientation_out
);

	localparam int unsigned EPI_FRAME_PTR_W = 2 * $clog2(IMAGE_DIM);
	...
	logic [$clog2(IMAGE_DIM)-1:0]        row_in_count     = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        column_in_count  = '0;
	...
	logic [$clog2(IMAGE_DIM)-1:0]        row_in_count_d     = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        row_in_count_dd    = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        column_in_count_d  = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        column_in_count_dd = '0;

	logic                           vertical_output_active = 1'b0;
	logic                           vertical_start_pulse   = 1'b0;

	logic [$clog2(IMAGE_DIM)-1:0]        v_epi_idx     = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        v_spatial_idx = '0;

	logic                           v_read_issue_d   = 1'b0;
	logic                           v_read_issue_dd  = 1'b0;
	logic [$clog2(IMAGE_DIM)-1:0]        v_epi_idx_d      = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        v_epi_idx_dd     = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        v_spatial_idx_d  = '0;
	logic [$clog2(IMAGE_DIM)-1:0]        v_spatial_idx_dd = '0;
	...
	logic [RAM_ADDR_W-1:0]          wr_addr_calc_off;
	logic [RAM_ADDR_W-1:0]          wr_addr_frame4_off;
	logic [RAM_ADDR_W-1:0]          wr_addr_transposed_off;
	logic [RAM_ADDR_W-1:0]          wr_addr_row_major_off;
	logic [RAM_ADDR_W-1:0]          rd_addr_h_live_off;
	logic [RAM_ADDR_W-1:0]          rd_addr_v_post_off;

	logic                           write_phase_q;
	logic                           v08_store_phase_q;
	logic [4:0]                     capture_in_count_q;
	logic [14:0]                    pixel_in_write_q;
	logic [RAM_ADDR_W-1:0]          wr_addr_calc_q;
	logic [RAM_ADDR_W-1:0]          wr_addr_frame4_q;
	logic [RAM_ADDR_W-1:0]          wr_addr_transposed_q;
	logic [RAM_ADDR_W-1:0]          wr_addr_row_major_q;

	logic                           storage_we_r [0:11];
	logic                           storage_we_8v_r;
	logic [RAM_ADDR_W-1:0]          storage_wr_addr_r [0:11];
	logic [RAM_ADDR_W-1:0]          storage_wr_addr_8v_r;
	...
	function automatic [EPI_FRAME_PTR_W-1:0] addr_transposed(
		...
		begin
			addr_transposed =
				({{(EPI_FRAME_PTR_W-$clog2(IMAGE_DIM)){1'b0}}, col_i} * IMAGE_DIM) +
				{{(EPI_FRAME_PTR_W-$clog2(IMAGE_DIM)){1'b0}}, row_i};
		end
	endfunction

	function automatic [RAM_ADDR_W-1:0] colour_addr(
		input logic [EPI_FRAME_PTR_W-1:0] local_addr_i
	);
		begin
			colour_addr = RAM_COLOUR_OFFSET +
			              {{(RAM_ADDR_W-EPI_FRAME_PTR_W){1'b0}}, local_addr_i};
		end
	endfunction
	...
	assign wr_addr_calc_off       = colour_addr(wr_addr_calc);
	assign wr_addr_frame4_off     = colour_addr(wr_addr_frame4);
	assign wr_addr_transposed_off = colour_addr(wr_addr_transposed);
	assign wr_addr_row_major_off  = colour_addr(wr_addr_row_major);
	assign rd_addr_h_live_off     = colour_addr(rd_addr_h_live);
	assign rd_addr_v_post_off     = colour_addr(rd_addr_v_post);

	assign storage_rd_addr = vertical_output_active ? rd_addr_v_post_off : rd_addr_h_live_off;
	...
	always_ff @(posedge clk) begin : Write_Command_Pipeline
		...
		wr_addr_calc_q       <= wr_addr_calc_off;
		wr_addr_frame4_q     <= wr_addr_frame4_off;
		wr_addr_transposed_q <= wr_addr_transposed_off;
		wr_addr_row_major_q  <= wr_addr_row_major_off;
	end
	...
	always_ff @(posedge clk) begin : Registered_Storage_Write_Drive
		...
		if (write_phase_q) begin
			case (capture_in_count_q)
				5'd0  : begin storage_we_r[0]  <= 1'b1;  storage_wr_addr_r[0]  <= wr_addr_calc_q;   end
				5'd1  : begin storage_we_r[1]  <= 1'b1;  storage_wr_addr_r[1]  <= wr_addr_calc_q;   end
				5'd2  : begin storage_we_r[2]  <= 1'b1;  storage_wr_addr_r[2]  <= wr_addr_calc_q;   end
				5'd3  : begin storage_we_r[3]  <= 1'b1;  storage_wr_addr_r[3]  <= wr_addr_calc_q;   end
				5'd4  : begin storage_we_r[4]  <= 1'b1;  storage_wr_addr_r[4]  <= wr_addr_frame4_q; end
				5'd5  : begin storage_we_r[5]  <= 1'b1;  storage_wr_addr_r[5]  <= wr_addr_calc_q;   end
				5'd6  : begin storage_we_r[6]  <= 1'b1;  storage_wr_addr_r[6]  <= wr_addr_calc_q;   end
				5'd7  : begin storage_we_r[7]  <= 1'b1;  storage_wr_addr_r[7]  <= wr_addr_calc_q;   end
				5'd8  : begin
					storage_we_r[8]       <= 1'b1;
				...
				5'd9  : begin storage_we_r[9]  <= 1'b1;  storage_wr_addr_r[9]  <= wr_addr_calc_q;   end
				5'd10 : begin storage_we_r[10] <= 1'b1;  storage_wr_addr_r[10] <= wr_addr_calc_q;   end
				5'd11 : begin storage_we_r[11] <= 1'b1;  storage_wr_addr_r[11] <= wr_addr_calc_q;   end
				5'd13 : begin storage_we_r[9]  <= 1'b1;  storage_wr_addr_r[9]  <= wr_addr_calc_q;   end
				5'd14 : begin storage_we_r[10] <= 1'b1;  storage_wr_addr_r[10] <= wr_addr_calc_q;   end
				5'd15 : begin storage_we_r[11] <= 1'b1;  storage_wr_addr_r[11] <= wr_addr_calc_q;   end
				default: begin end
			endcase
		end
	...
	always_ff @(posedge clk) begin : LF_Control_And_Addressing
		...
		if ((in_lf_flag || solf_in) && pixel_valid_in) begin
			...
			if (soc_in) begin
				row_in_count    <= '0;
				column_in_count <= {{($clog2(IMAGE_DIM)-1){1'b0}}, 1'b1};
			end
			else begin
				if (column_in_count == IMAGE_DIM-1) begin
					column_in_count <= '0;
					row_in_count    <= row_in_count + {{($clog2(IMAGE_DIM)-1){1'b0}}, 1'b1};
				end
				else begin
					column_in_count <= column_in_count + {{($clog2(IMAGE_DIM)-1){1'b0}}, 1'b1};
				end
			end
			...
			if (vertical_start_pulse) begin
				...
				if (v_spatial_idx == IMAGE_DIM-1) begin
					...
					if (v_epi_idx == IMAGE_DIM-1) begin
						...
						v_epi_idx <= v_epi_idx + {{($clog2(IMAGE_DIM)-1){1'b0}}, 1'b1};
					end
				end
				else begin
					v_spatial_idx <= v_spatial_idx + {{($clog2(IMAGE_DIM)-1){1'b0}}, 1'b1};
				end
			end
		end
	end

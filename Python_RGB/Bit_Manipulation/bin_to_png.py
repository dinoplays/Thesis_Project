...
def _decode_u24_q12_12(payload: bytes, n_samples: int) -> np.ndarray:
    ...
    # (u - BIAS_INT) / 2^Q_FRAC
    # Use power-of-two scaling via numpy.ldexp:
    # ldexp(x, -Q_FRAC) == x / (2^Q_FRAC)
...
def read_imgb(path_in: str) -> tuple[np.ndarray, int]:
    ...
    expected = W * H * C * bytes_per_sample
...
def convert_folder_imgb_to_png(in_dir: str) -> tuple[str, str]:
    ...
    print(f"Converting folder: {in_dir}")
    print(f"  IMGB files found: {len(names)}")
    ...
    for name in names:
        ...
        linear_path = os.path.join(out_linear, base + ".png")
        iio.imwrite(linear_path, linear)
        ...
        robust_path = os.path.join(out_robust, base + ".png")
        iio.imwrite(robust_path, robust)
        print(f"Saved to: {robust_path}")
...
def write_reliable_outputs(
    ...
    Uses pink-mask plot with robust grayscale limits.
    """
...
if __name__ == "__main__":
    convert_scene_imgb_to_png(
        scene_dir="Python_RGB/Bit_Manipulation/head",
        reliable_thresh=0.3,

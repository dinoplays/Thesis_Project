module low_pass_filter #(
    ...
    logic [$clog2(IMAGE_DIM)-1:0] row_out_count    = '0;
    logic [$clog2(IMAGE_DIM)-1:0] column_out_count = '0;
    ...
    always_ff @(posedge clk) begin : Image_Buffer
        ...
        if (eoc_lag_flag && (((row_in_count == '0) && (column_in_count == '0)) || pixel_valid_in)) begin
            ...
            if (end_lag_buffer_count == (3 * IMAGE_DIM) + 2) begin
                eoc_out_pulse <= 1'b1;
            end

            if (end_lag_buffer_count == (3 * IMAGE_DIM) + 3) begin
                eoc_lag_flag <= 1'b0;
            end
        end
        ...
        if (pixel_valid_in || (eoc_lag_flag && (row_in_count == '0) && (column_in_count == '0))) begin
            ...
            if (pixel_valid_in) begin
                ...
                if (!soc_lag_flag) begin
                    ...
                    if (start_lag_buffer_count == (3 * IMAGE_DIM) + 3) begin
                        soc_lag_flag  <= 1'b1;
                        soc_out_pulse <= 1'b1;
                    end
                end
    ...
    always_ff @(posedge clk) begin : Convolution_Pipeline
        ...
        raw_s0 <= {pixel_buffer[(3 * IMAGE_DIM) + 3], 7'd0};
        ...
        for (idx = 0; idx < 24; idx = idx + 1) begin
            sum_s1[idx] <= {1'b0, tap_s0[(idx * 2)]} + {1'b0, tap_s0[(idx * 2) + 1]};
        end
        ...
        for (idx = 0; idx < 12; idx = idx + 1) begin
            sum_s2[idx] <= {1'b0, sum_s1[(idx * 2)]} + {1'b0, sum_s1[(idx * 2) + 1]};
        end
        ...
        for (idx = 0; idx < 6; idx = idx + 1) begin
            sum_s3[idx] <= {1'b0, sum_s2[(idx * 2)]} + {1'b0, sum_s2[(idx * 2) + 1]};
        end
        ...
        for (idx = 0; idx < 3; idx = idx + 1) begin
            sum_s4[idx] <= {1'b0, sum_s3[(idx * 2)]} + {1'b0, sum_s3[(idx * 2) + 1]};
        end
        ...
        if (eoc_now) begin
            ...
            if (column_out_count == IMAGE_DIM - 1) begin
                column_out_count <= '0;
                row_out_count    <= row_out_count + {{($clog2(IMAGE_DIM)-1){1'b0}}, 1'b1};
            end
            else begin
                column_out_count <= column_out_count + {{($clog2(IMAGE_DIM)-1){1'b0}}, 1'b1};
            end
        end
    end

...
#     2) no-confidence average fusion for testing
...
# ---------------- fusion (confidence-weighted, original) ----------------
...
# ---------------- fusion (NO confidence: simple average) ----------------

def fuse_disparity_average_no_confidence(
    Z_h_imgb: bytes,
    Z_v_imgb: bytes,
) -> bytes:
    """
    Fuse horizontal and vertical disparity without confidence.

    Computes:
        Z_avg = (Z_h + Z_v) / 2

    Time complexity:
    - O(H * W), one loop over all pixels.
    """
    W1, H1, C1, dt1, pZh = imgb_parse(Z_h_imgb)
    W2, H2, C2, dt2, pZv = imgb_parse(Z_v_imgb)

    if W1 != W2 or H1 != H2:
        raise ValueError("average fusion: dimension mismatch")

    if C1 != 1 or C2 != 1 or dt1 != 4 or dt2 != 4:
        raise ValueError("average fusion expects dtype_code=4, C=1")
    ...
    for i in range(n):
        zh_q = _u24_read(pZh, i * 3) - BIAS_INT
        zv_q = _u24_read(pZv, i * 3) - BIAS_INT

        z_avg_q = _round_div2(zh_q + zv_q)

        _u24_write(out_pay, i * 3, _bias_q(z_avg_q))

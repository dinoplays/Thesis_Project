module fused_aligned_output #(
	...
	output logic [((2*$clog2(IMAGE_DIM))-1):0]         shared_wr_addr [0:11],
	output logic [14:0]                           shared_wr_data [0:11],
	output logic [((2*$clog2(IMAGE_DIM))-1):0]         shared_rd_addr [0:11],
	...
	always_ff @(posedge clk) begin : Register_Aligned_Recombine_Terms
		...
		if (fuse_pair_valid_5r) begin
			...
			weighted_term_h_hi_sh_5x_red <= ($signed({{11{weighted_term_h_hi_5r_red[27]}}, weighted_term_h_hi_5r_red}) * 4096);
			weighted_term_v_lo_5x_red    <= $signed({12'd0, weighted_term_v_lo_5r_red});
			weighted_term_v_hi_sh_5x_red <= ($signed({{11{weighted_term_v_hi_5r_red[27]}}, weighted_term_v_hi_5r_red}) * 4096);

			weighted_term_h_lo_5x_green    <= $signed({12'd0, weighted_term_h_lo_5r_green});
			weighted_term_h_hi_sh_5x_green <= ($signed({{11{weighted_term_h_hi_5r_green[27]}}, weighted_term_h_hi_5r_green}) * 4096);
			weighted_term_v_lo_5x_green    <= $signed({12'd0, weighted_term_v_lo_5r_green});
			weighted_term_v_hi_sh_5x_green <= ($signed({{11{weighted_term_v_hi_5r_green[27]}}, weighted_term_v_hi_5r_green}) * 4096);

			weighted_term_h_lo_5x_blue    <= $signed({12'd0, weighted_term_h_lo_5r_blue});
			weighted_term_h_hi_sh_5x_blue <= ($signed({{11{weighted_term_h_hi_5r_blue[27]}}, weighted_term_h_hi_5r_blue}) * 4096);
			weighted_term_v_lo_5x_blue    <= $signed({12'd0, weighted_term_v_lo_5r_blue});
			weighted_term_v_hi_sh_5x_blue <= ($signed({{11{weighted_term_v_hi_5r_blue[27]}}, weighted_term_v_hi_5r_blue}) * 4096);
		end
	end

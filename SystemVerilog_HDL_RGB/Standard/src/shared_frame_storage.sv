module shared_frame_storage #(
	...
	input  wire [RAM_ADDR_W-1:0]                  epi_wr_addr [0:11],
	input  wire [RAM_ADDR_W-1:0]                  epi_wr_addr_8v,
	input  wire [14:0]                            epi_wr_data,
	input  wire [RAM_ADDR_W-1:0]                  epi_rd_addr,
	output logic [14:0]                           epi_rd_data [0:11],
	output logic [14:0]                           epi_rd_data_8v,

	input  wire                                   fao_we [0:3],
	input  wire [RAM_ADDR_W-1:0]                  fao_wr_addr [0:3],
	input  wire [14:0]                            fao_wr_data [0:3],
	input  wire [RAM_ADDR_W-1:0]                  fao_rd_addr [0:3],
	output logic [14:0]                           fao_rd_data [0:3]
);

	localparam int unsigned FRAME_SIZE   = IMAGE_DIM * IMAGE_DIM;
	localparam int unsigned ADDR_W       = RAM_ADDR_W;
	...
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_0  (.clk(clk), .we(bank0_we_q),  .wr_addr(bank0_wr_addr_q),  .wr_data(bank0_wr_data_q),  .rd_addr(bank0_rd_addr_q),  .rd_data(epi_rd_data[0]));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_1  (.clk(clk), .we(bank1_we_q),  .wr_addr(bank1_wr_addr_q),  .wr_data(bank1_wr_data_q),  .rd_addr(bank1_rd_addr_q),  .rd_data(epi_rd_data[1]));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_2  (.clk(clk), .we(bank2_we_q),  .wr_addr(bank2_wr_addr_q),  .wr_data(bank2_wr_data_q),  .rd_addr(bank2_rd_addr_q),  .rd_data(epi_rd_data[2]));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_3  (.clk(clk), .we(bank3_we_q),  .wr_addr(bank3_wr_addr_q),  .wr_data(bank3_wr_data_q),  .rd_addr(bank3_rd_addr_q),  .rd_data(epi_rd_data[3]));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_4  (.clk(clk), .we(bank4_we_q),  .wr_addr(bank4_wr_addr_q),  .wr_data(bank4_wr_data_q),  .rd_addr(bank4_rd_addr_q),  .rd_data(epi_rd_data[4]));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_8v (.clk(clk), .we(bank8v_we_q), .wr_addr(bank8v_wr_addr_q), .wr_data(bank8v_wr_data_q), .rd_addr(bank8v_rd_addr_q), .rd_data(epi_rd_data_8v));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_9  (.clk(clk), .we(bank9_we_q),  .wr_addr(bank9_wr_addr_q),  .wr_data(bank9_wr_data_q),  .rd_addr(bank9_rd_addr_q),  .rd_data(epi_rd_data[9]));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_10 (.clk(clk), .we(bank10_we_q), .wr_addr(bank10_wr_addr_q), .wr_data(bank10_wr_data_q), .rd_addr(bank10_rd_addr_q), .rd_data(epi_rd_data[10]));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_11 (.clk(clk), .we(bank11_we_q), .wr_addr(bank11_wr_addr_q), .wr_data(bank11_wr_data_q), .rd_addr(bank11_rd_addr_q), .rd_data(epi_rd_data[11]));
	...
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_5 (.clk(clk), .we(bank5_we_q), .wr_addr(bank5_wr_addr_q), .wr_data(bank5_wr_data_q), .rd_addr(bank5_rd_addr_q), .rd_data(bank5_rd_data));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_6 (.clk(clk), .we(bank6_we_q), .wr_addr(bank6_wr_addr_q), .wr_data(bank6_wr_data_q), .rd_addr(bank6_rd_addr_q), .rd_data(bank6_rd_data));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_7 (.clk(clk), .we(bank7_we_q), .wr_addr(bank7_wr_addr_q), .wr_data(bank7_wr_data_q), .rd_addr(bank7_rd_addr_q), .rd_data(bank7_rd_data));
	frame_ram #(.DATA_W(15), .DEPTH(FRAME_SIZE), .ADDR_W(ADDR_W)) bank_8 (.clk(clk), .we(bank8_we_q), .wr_addr(bank8_wr_addr_q), .wr_data(bank8_wr_data_q), .rd_addr(bank8_rd_addr_q), .rd_data(bank8_rd_data));
